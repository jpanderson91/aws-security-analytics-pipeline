...
Windows-compatible validation with proper path handling and encoding
"""
...
class CAPDemoValidator:
    ...
    def __init__(self):
        ...
        self.env['AWS_PROFILE'] = self.profile
    ...
    def test_script_compatibility(self):
        ...
        try:
            ...
            result = subprocess.run([
                'python', str(phase1_script), '--help'
            ], capture_output=True, text=True, 
            env=self.env, timeout=30, check=False)
            
            if result.returncode == 0:
                compatibility_results['Phase 1'] = "✅ Compatible"
            else:
                compatibility_results['Phase 1'] = f"⚠️ Issues detected"
                print(f"   Issues: {result.stderr[:200]}...")
                
        except subprocess.TimeoutExpired:
            compatibility_results['Phase 1'] = "⚠️ Timeout (expected)"
        except Exception as e:
            compatibility_results['Phase 1'] = f"❌ Error: {e}"
        
        # Test script syntax compilation
        scripts_to_test = [
            ('Phase 2', 'setup_phase2_processing.py'),
            ('Phase 3', 'setup_phase3_analytics.py'),
            ('Validation', 'run_complete_validation.py')
        ]
        
        for phase_name, script_name in scripts_to_test:
